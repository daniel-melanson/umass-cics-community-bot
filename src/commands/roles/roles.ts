...
export const isResidential = matcher(
  /^(central|ohill|northeast|southwest|honors|sylvan|off-campus)$/i,
);
export const isGraduationStatus = matcher(
  /^(alumni|graduate student|class of \d{4})$/i,
);
export const isInterdisciplinary = matcher(
  /^(business|linguistics|physics|psychology)$/i,
);
export const isMisc = matcher(
  /^(snooper|daily coding problems|community events)$/i,
);
